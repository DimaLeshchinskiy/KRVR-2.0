...
class IntPerimeterActionProcess:
    def __init__(self):
        self.toolWidth = 0
        self.toolLength = 0
        self.wallOffset = 0
    ...
    def getLines(self, polygon):
        ...
        while currentOffset >= self.toolWidth / 2:
            ...
            if currentOffset < minOffset:
                lines.append(exterior.parallel_offset(minOffset, side="right", join_style=2).coords)
                break
            else:
                lines.append(exterior.parallel_offset(currentOffset, side="right", join_style=2).coords)
        return lines
    ...
    def makeGcode(self, data=None, action=None, tool=None, material=None, objectOptions=None) -> GrblGcodeBuilder:
        ...
        while currentHeight > minY:
            ...
            if currentHeight > minY:
                mainGcodeBuilder.appendBuilder(self.milling(currentHeight, lines), comment="Interior perimeter on y=" + str(currentHeight))
            else:
                mainGcodeBuilder.appendBuilder(self.milling(minY, lines), comment="Interior perimeter on y=" + str(minY))
